  ...
  runDetection(){
    if (this.model != null){
        let predictions = this.model.detect(this.video.nativeElement).then((predictions: any) => {
            ...
            //custom gesture 2
            if (pointing == 1 && closedhands == 1) this.detectedGesture = "One Hand Pointing & One Closed Hand";

            if (openhands == 0 && closedhands == 0 && pointing == 0 && pinching == 0)
                this.detectedGesture = "None";

            this.onPrediction.emit(new PredictionEvent(this.detectedGesture, xpos, ypos))
        }, (err: any) => {
            console.log("ERROR")
            console.log(err)
        });
